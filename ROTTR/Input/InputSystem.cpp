...
static const float g_controllerBindThreshold = 0.5f;
...
{
  // Processes keyboard + gamepad input and updates action states based on bindings.
    ...
    if (m_Gamepad.IsPresent)
    {
      ...
      if (m_CaptureState.CaptureGamepad)
      {
        for (int i = 0; i < GamepadKey::GamepadKey_Count; ++i)
        {
          ...
          if (keyState > g_controllerBindThreshold)
          {
            m_CaptureState.CaptureGamepad = false;
...
void InputSystem::UpdateXInput()
{
...

  {
    ...
    float trigger = (static_cast<float>(delta)) / (255.0 - XINPUT_GAMEPAD_TRIGGER_THRESHOLD / 2);
    if (trigger > 0)
