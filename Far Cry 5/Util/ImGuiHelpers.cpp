...
bool ImGui::ColorPicker4(const char* label, float col[4], float* pIntensity, float intensityCol[4], ImGuiColorEditFlags flags, const float* ref_col)
{
  ...
  if ((flags & ImGuiColorEditFlags_NoInputs) == 0)
  {
    ...
    if (pIntensity != nullptr)
      intensity_changed |= InputFloat("Intensity", pIntensity, 0.1f, 1.0f, 2);
    PopItemWidth();
  }
  ...
  return intensity_changed || (value_changed && memcmp(backup_initial_col, col, components * sizeof(float)));
}
