...
Dx11Renderer::Dx11Renderer()
{
  ...
  m_pDepthConstantBuffer = nullptr;
  m_pDepthPixelShader = nullptr;
...
bool Dx11Renderer::Initialize(ID3D11Device* pDevice, HWND hwnd)
{
  ...
  util::log::Write("[Dx11Renderer] ID3D11Device 0x%X", m_pDevice);
  util::log::Write("[Dx11Renderer] ID3D11DeviceContext 0x%X", m_pContext);
  ...
  m_pPrimitiveBatch = std::make_unique<PrimitiveBatch<VertexPositionColor>>(m_pContext);
  m_pSpriteBatch = std::make_unique<SpriteBatch>(m_pContext);
  m_pFxFactory = std::make_unique<EffectFactory>(m_pDevice);
  m_pEffect3DPrimitive = std::make_unique<BasicEffect>(m_pDevice);
  m_pEffect3DModel = std::make_unique<BasicEffect>(m_pDevice);
  m_pEffect3DShape = std::make_unique<BasicEffect>(m_pDevice);
  ...
  DWORD szShader = 0;
  /*
  if (!util::GetResource(IDR_ZBUFFERSHADER, pShaderData, szShader))
  {
    util::log::Error("Could not load ZBuffer Shader from resources");
    return false;
  }

  hr = fb::Dx11Renderer::Singleton()->m_pDevice->CreatePixelShader(pShaderData, szShader, NULL, &m_pDepthPixelShader);
  if (FAILED(hr))
  {
    util::log::Error("Failed to create ZBuffer shader, HRESULT 0x%X GetLastError 0x%X", hr, GetLastError());
    return false;
  }
  ...
  InitData.pSysMem = &m_depthConstants;
  InitData.SysMemPitch = 0;
  InitData.SysMemSlicePitch = 0;

  hr = fb::Dx11Renderer::Singleton()->m_pDevice->CreateBuffer(&cbDesc, &InitData, &m_pDepthConstantBuffer);
  if (FAILED(hr))
  {
    util::log::Error("Failed to create constant buffer, HRESULT 0x%X GetLastError 0x%X", hr, GetLastError());
    return false;
  }
  */
